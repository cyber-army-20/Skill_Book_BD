...
function buildCard(product) {
  ...
  const buyUrl = promoActive ? product.promo_link : product.affiliate_url;

  card.innerHTML = `
    <div class="relative bg-ink/5">
      <img src="${escapeAttr(product.image)}" alt="${escapeAttr(product.title)}"
           class="w-full aspect-video object-contain" loading="lazy"
    ...
    <div class="p-4 flex flex-col flex-1">
      ...
      <div class="flex items-center flex-wrap gap-2 mb-3">
        ${hasDiscount ? `
          <span class="text-lg font-bold text-teal">${priceLabel(product.discounted_price)}</span>
          <span class="text-sm text-ink/40 line-through">${priceLabel(product.original_price)}</span>
        ` : `
          <span class="text-lg font-bold text-ink">${priceLabel(product.original_price)}</span>
        `}
        ${promoActive ? `
      ...
      <div class="mt-auto pt-3 flex items-center gap-2">
        <a href="${escapeAttr(buyUrl)}" target="_blank" rel="noopener noreferrer"
...
function renderSingle(product) {
  ...
  els.singleView.innerHTML = `
    ...
    <div class="bg-white rounded-2xl shadow-sm ring-1 ring-ink/5 overflow-hidden grid md:grid-cols-2">
      <div class="relative bg-ink/5">
        <img src="${escapeAttr(product.image)}" alt="${escapeAttr(product.title)}"
             class="w-full h-64 md:h-full object-contain"
             onerror="this.src='https://placehold.co/600x400/132239/FBF7EE?text=Skill+%26+Book+BD'"/>
      </div>
      ...
      <div class="p-6 sm:p-8 flex flex-col">
        ...
        <div class="flex items-center flex-wrap gap-3 mb-5">
          ${hasDiscount ? `
            <span class="text-2xl font-bold text-teal">${priceLabel(product.discounted_price)}</span>
            <span class="text-base text-ink/40 line-through">${priceLabel(product.original_price)}</span>
          ` : `
            <span class="text-2xl font-bold text-ink">${priceLabel(product.original_price)}</span>
          `}
          ${promoActive ? `
...
function isPromoActive(product) {
  if (!isPresent(product.promo_code) || !isPresent(product.promo_link)) return false;
  // A promo with no expiry date set is treated as always active.
  if (!isPresent(product.promo_expiry)) return true;
