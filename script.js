...
function buildCard(product) {
  ...
  card.innerHTML = `
    <div class="relative">
      ...
      ${hasPromo ? `
      <span class="absolute top-3 left-3 bg-rose text-white text-[11px] font-semibold px-2.5 py-1 rounded-full shadow-sm tracking-wide">
    ...
    <div class="bg-white rounded-2xl shadow-sm ring-1 ring-ink/5 overflow-hidden grid md:grid-cols-2">
      <div class="relative">
        <img src="${escapeAttr(product.image)}" alt="${escapeAttr(product.title)}"
             ...
             onerror="this.src='https://placehold.co/600x400/132239/FBF7EE?text=Skill+%26+Book+BD'"/>
        ${hasPromo ? `
        <span class="absolute top-4 left-4 bg-rose text-white text-xs font-semibold px-3 py-1.5 rounded-full shadow-sm tracking-wide">
          CODE: ${escapeHtml(product.promo_code)}
        </span>` : ''}
      </div>
